# Orchestra Pipeline Agents

Orchestra provides a suite of AI-powered agents designed to automate various stages of your software development lifecycle. These agents can be integrated into your CI/CD pipelines to handle tasks ranging from code review to security scanning.
...
*   **Command:** `orchestra vuln-scan <path_to_file_or_dir>`
...
*   **Command:** `orchestra pr-review <pr_number>`
...
*   **Command:** `orchestra dev-cycle <file_path> "<task_description>"`
...
*   **Use Case:** Trigger when a new issue is labeled `orchestra`.
*   **Command:** `orchestra agile <issue_number> "<issue_title>"`
...
*   **Command:** `orchestra release-notes <version>`
...
*   **Command:** `orchestra fix <file_path> "[instruction]"`
...
*   **Command:** `orchestra project-manager <requirement_or_file> "[instruction]"`
...
*   **Command:** `orchestra test-gen <file_path>`
...
*   **Command:** `orchestra doc-gen <file_path>`

---

## 🔧 General Usage in Pipelines

To use Orchestra in any pipeline:

1.  **Install Orchestra:** Ensure `orchestra` is installed and available in your path (e.g., `npm install -g orchestra-svemind` or run via `npx`).
